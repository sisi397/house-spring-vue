...
function displayAPIMarkers(places){
	...
	for(var i=0; i<places.length; i++) {
		...
		var addr = places[i].sidoName+" "+places[i].gugunName+" "+places[i].dongName+" "+places[i].jibun;
		    ...
		    // 정상적으로 검색이 완료됐으면 
		     if (status === kakao.maps.services.Status.OK) {
		      ...
		      /*  var infowindow = new kakao.maps.InfoWindow({
		            content: `<div style="width:150px;text-align:center;padding:6px 0;">${places[i].aptName}</div>`
		        });
...
function addMarker(position, idx, title) {
	...
	marker = new kakao.maps.Marker({
		position: position, // 마커의 위치. 위도, 경도
		image: markerImage,
	});
