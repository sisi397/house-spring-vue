<template>
  <b-container class="bv-example-row mt-3 text-center">
    <b-container class="button-group" style="margin-bottom: 20px">
      <b-button pill variant="primary" @click="displayMarker()" style="margin-right: 10px"
        >위치 검색</b-button
      >
          ...
          <b-table-simple striped hover responsive>
            ...
            <tbody>
              ...
              <b-tr v-for="(facility, index) in facilities" :key="index">
                ...
                <b-td class="content">{{ facility.distance }}m</b-td>
              </b-tr>
            </tbody>
          </b-table-simple>
        </div>
...
export default {
  ...
  methods: {
    ...
    displayMarker() {
      ...
      if (this.flag === 1) {
        // 하나만 표시
        positions = new kakao.maps.LatLng(this.houseOne.lat, this.houseOne.lng);
      }

      // const positions = markerPositions.map((position) => new kakao.maps.LatLng(...position));
      console.log(positions);

      this.aptDBMarkers.push(
    ...
    searchFacilities() {
      ...
      function displayPlaces(places) {
        ...
        for (var i = 0; i < places.length; i++) {
          ...
          _this.facilities.push({
            ...
            distance: getDistance(places[i].y, places[i].x), // 거리 구하기
          });
        ...
        // 거리 짧은 순으로 정렬
        _this.facilities = _this.facilities.sort((a, b) => {
      ...
      // 직선 거리 계산
      function getDistance(f_lat, f_lng) {
        let polyline = new kakao.maps.Polyline({
          map: _this.map,
          path: [
            new kakao.maps.LatLng(_this.houseOne.lat, _this.houseOne.lng),
            new kakao.maps.LatLng(f_lat, f_lng),
          ],
          strokeWeight: 2,
          strokeColor: "#FF00FF",
          strokeOpacity: 0.8,
      ...
      function onClickCategory() {
        // 지도에서 편의시설과 집 사이 선이 있다면 지우기
        for (let i = 0; i < _this.polylines.length; ++i) {
          _this.polylines[i].setMap(null);
          _this.polylines[i] = null;
        }
        _this.polylines = []; // 거리 배열 비우기

        _this.facilities = []; // 편의 시설 배열 비우기
        var id = this.id,
